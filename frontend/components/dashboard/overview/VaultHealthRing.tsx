...
export function VaultHealthRing({ score, isLoading }: Props) {
    return (
        <Link href="/dashboard/health" className="col-span-1 bg-card rounded-2xl p-6 border border-border hover:bg-muted/50 transition-colors group flex flex-col">
                ...
                <div className="flex flex-col items-center py-4">
                    <div className="relative w-32 h-32">
                        ...
                        <div className="absolute inset-0 flex items-center justify-center">
                            <motion.span className={`text-3xl font-bold font-sans text-foreground`} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }}>
                                {score}
                            </motion.span>
                        </div>
                    </div>
                    <span className={`mt-3 text-xs font-semibold px-3 py-1 rounded-full bg-muted text-foreground border border-border`}>
                        {healthLabel(score)}
                    </span>
